CREATE PROCEDURE [etl].[spInsertMetadata]
...
SET @SystemActive = (SELECT TOP 1 [Active] FROM [etl].[MetadataSystem] 
						WHERE 1=1 
...
IF EXISTS (SELECT TOP 1 [IsActive] FROM [etl].[MetadataTables]
					WHERE 1=1 
					AND [SystemCode] = @SystemCode
					AND [SystemName] = @SystemName
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
Adding the records of the source system to the [etl].[MetadataTables] table. 
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	IF @SystemActive = 1
	BEGIN
	EXECUTE [etl].[spInsertTable] @systemcode,@systemname,@lschema,@lTable;
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
Adding the records of the source system to the [etl].[MetadataStructure] table
  ...
  PRINT CHAR(13) + '*** Begin Transaction ***'
	...
	IF @TableActive = 1
			...
			/* Update Removed Columns in source to IsActive = 0 and Is Deleted = 1 if Column not in Json*/ 
				PRINT CHAR(13) + '*** Update Columns in etl.MetadataStructure that are not in the source ***'				
								
						UPDATE [etl].[MetadataStructure] WITH (ROWLOCK)  --Via Merge proberen met andere Update!!
						SET   IsActive  = 0
							, IsDeleted = 1
			...
			END
			PRINT CHAR(13) + '*** Succesfully Update Columns in etl.MetadataStructure that are not in the source ***';
	...
	IF @TableActive = 1
			...
			/*Delete All Active rows that will Be updated from the Metadata Table */
				PRINT CHAR(13) + '*** Delete IsActive recods from etl.MetadataStructure  ***'
		
					DELETE FROM [etl].[MetadataStructure] With (ROWLOCK)
					WHERE   [SystemCode] = @SystemCode
						AND [SystemName] = @SystemName
						AND [SchemaName] = @lSchema
						AND [EntityName] = @lTable
	...
	PRINT CHAR(13) + '*** Execute spInsertStructure' + @SystemType + ' ***'
		...
		----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				EXECUTE  [etl].[spInsertStructureMySQL] @systemcode,@SystemName, @lSchema, @lTable, @Json
		...
		----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				EXECUTE  [etl].[spInsertStructureSQL] @systemcode,@SystemName, @lSchema, @lTable, @Json
		...
		----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				EXECUTE [etl].[spInsertStructureSalesForce] @systemcode, @SystemName ,@lSchema, @lTable, @Json
		...
		----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				EXECUTE [etl].[spInsertStructureJson] @systemcode, @SystemName ,@lSchema, @lTable, @Json
		...
		----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				EXECUTE  [etl].[spInsertStructureOracle] @systemcode,@lSchema, @lTable, @Json
	...
	--AND EXISTS (select top 1 * from #TempColumns) 	
		...
		-- Set PK, Semanctic and metadata Values to values from before run
					UPDATE [etl].[MetadataStructure] WITH (ROWLOCK)
					SET  IsPrimaryKey = T.IsPrimaryKey
						,SemanticName = T.SemanticName
						,IsHistory    = T.IsHistory
						,IsDeleted    = T.IsDeleted
						,IsActive     = CASE WHEN NOT EXISTS (Select [Name] FROM [etl].[MetadataStructure]
					...
					FROM #TempColumns T
					LEFT JOIN [etl].[MetadataStructure] M
