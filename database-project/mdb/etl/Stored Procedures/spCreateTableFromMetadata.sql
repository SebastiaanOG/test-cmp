...
CREATE PROCEDURE [etl].[spCreateTableFromMetadata]
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION
			...
			SELECT @drop_statement = CONCAT('DROP TABLE ', [etl].[fnCreateTableName](@system_name, @entity_name))

			SELECT @create_statement = CONCAT(
										'CREATE TABLE '
										, [etl].[fnCreateTableName](@system_name, @entity_name)
										, '('
										, STRING_AGG(
			...
			FROM [etl].[vwMetaData] t
			WHERE 1=1
			AND [etl].[fnCreateTableName](t.SystemName, t.EntityName) = [etl].[fnCreateTableName](@system_name, @entity_name)
...
--			DvB: 02-10-2020 Toevoeging dat alleen wanneer de tabel actief is opnieuw wordt opgebouwd.
	...
	IF (Select [IsActive] from [etl].[MetadataTables] where @entity_name = EntityName AND SystemName = @system_name) = 1
			BEGIN
			IF NOT EXISTS
			(
				SELECT *
				...
				WHERE 1=1
				AND [etl].[fnCreateTableName](TABLE_SCHEMA, TABLE_NAME) = [etl].[fnCreateTableName](@system_name, @entity_name)
			)
			BEGIN
			PRINT 1
