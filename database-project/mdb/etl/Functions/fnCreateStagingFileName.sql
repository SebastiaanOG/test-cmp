CREATE FUNCTION [etl].[fnCreateStagingFileName](
...
AS
     BEGIN
         ...
         SET @result = CASE
                           WHEN @IncrementColumnName IS NOT NULL /*AND @deltadate IS NOT NULL*/ THEN CONCAT(@schemaname, '.', LOWER(@entityname), '_increment', '.parquet')						   
         ...
         --                                                END, '.parquet')
                           ELSE CONCAT(@schemaname, '.', LOWER(@entityname), '_full', '.parquet')
                       END;
         RETURN @result;
     END