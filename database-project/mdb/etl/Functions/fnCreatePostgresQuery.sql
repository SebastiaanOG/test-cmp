CREATE FUNCTION [etl].[fnCreatePostgresQuery] 
	(
...
BEGIN
	...
	SELECT @where_clause = 
		CASE
			...
			THEN CONCAT('WHERE', CHAR(32), '"', @delta_name, '"', CHAR(32), 'BETWEEN', CHAR(32), 
			IIF(@delta_range > 0, CONCAT('''',  @delta_date, '''', CHAR(32), 'AND', CHAR(32),  '''', DATEADD(day, @delta_range, @delta_date), ''''), CONCAT( '''', DATEADD(day, @delta_range, @delta_date), '''',  CHAR(32), 'AND',  CHAR(32),  '''', @delta_date, '''')))
            WHEN @delta_name IS NOT NULL
             AND @delta_date IS NOT NULL
             AND @delta_range IS NULL 
            THEN CONCAT('WHERE', CHAR(32), '"', @delta_name, '"', CHAR(32), '=', CHAR(32),  '''', @delta_date, '''')
            ELSE NULL
        END;
