/*
    Stored Procedure template for dimension or fact with append. 
    
    Type 2 - new row on change

    - NOT MATCHED IN TARGET : Inserts data for new application key (ak)
    - MATCHED > Inserts data for changed hashes :
        > When records are matched, add new record with new valid_from and valid_to
        > Invalidate record with previous key
    ...
    TODO : if a source record is active, then inactive, then active. What to do? Discuss with Sabine
...
BEGIN
    ...
    MERGE modelled.[$Destination] AS DESTINATION
    ...
    DECLARE @inserted_table TABLE
    (
        inserted_[ak_${field_ak}] VARCHAR(50) NULL
    );

    -- invalidate old records
    INSERT INTO modelled.[$Destination]
    (
        dwh_valid_from 
        ,dwh_valid_to
        ,dwh_active
    ...
    OUTPUT inserted.[ak_${field_ak}] INTO @inserted_table
    ...
    WHERE
        [ak_${field_ak}] in (
            SELECT inserted_[ak_${field_ak}]
            FROM @merge_results 
            WHERE -- in case of update with match, perform insert
                action_type = 'UPDATE' AND 
                inserted_ak_project IS NOT NULL AND 
                deleted_ak_project IS NOT NULL
            )  
   
    COMMIT

    SELECT @deleted = COUNT(DELETED.[ak_${field_ak}]) FROM @merge_results WHERE action_type = 'DELETE'
    SET @inserted = (SELECT COUNT(inserted_[ak_${field_ak}]) FROM @merge_results WHERE action_type = 'INSERT') + (SELECT COUNT(*) FROM @inserted_table)
