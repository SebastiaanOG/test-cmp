...
,@IndFirstDayOfMonth	bit
,@IndLastDayOfMonth     bit
,@IndWeekDay 		    bit
,@IndWeekendDay			bit
,@IndDutchHolliday      bit
	...
	begin
		...
		while @LoopYear = Year(@LoopDate)
			begin
				...
				if @dayNumberMonth = 1 set @IndFirstdayofMonth = 1 else set @IndFirstdayofMonth = 0
				if DATEPART(DAY,DATEADD(D,1,@LoopDate)) = 1 set @IndLastDayOfMonth = 1 else set @IndLastdayofMonth = 0
				if SUBSTRING(@dayName,1,2) in ('sa', 'su') set @IndWeekendday = 1 else set @IndWeekendday = 0
				if SUBSTRING(@dayName,1,2) not in ('sa', 'su')  set @IndWeekday = 1 else set @IndWeekday = 0
				--- Holiday's ---
				set @IndDutchHolliday = 0
				set @HolidayName = NULL
				if (@dayNumberYear = 1 and @MonthNumber = 1) set @IndDutchHolliday = 1
				if (@dayNumberYear = 1 and @MonthNumber = 1) set @HolidayName = 'New Year''s Day'
				if @LoopDate = @EasterDate set @IndDutchHolliday = 1
				if @LoopDate = @EasterDate set @HolidayName = 'Easter day' 
				if @LoopDate = DATEADD(D,1,@EasterDate) set @IndDutchHolliday = 1
				if @LoopDate = DATEADD(D,1,@EasterDate) set @HolidayName = 'Easter Monday' 
				if (@dayNumberMonth = 27 and @MonthNumber = 4 and SUBSTRING((LOWER(DATENAME(WEEKDAY,@LoopDate))),1,2) <> 'su') set  @IndDutchHolliday = 1
				if (@dayNumberMonth = 27 and @MonthNumber = 4 and SUBSTRING((LOWER(DATENAME(WEEKDAY,@LoopDate))),1,2) <> 'su') set @HolidayName = 'King''s day'
				if (@dayNumberMonth = 26 and @MonthNumber = 4 and SUBSTRING((LOWER(DATENAME(WEEKDAY,dateadd(day, 1,@LoopDate)))),1,2) = 'su') set  @IndDutchHolliday = 1
				...
				if (@dayNumberMonth = 5 and @MonthNumber = 5 and @Year % 5 = 0) set  @IndDutchHolliday = 1
				if (@dayNumberMonth = 5 and @MonthNumber = 5 and @Year % 5 = 0) set  @HolidayName = 'Liberation Day'
				
				if @LoopDate = DATEADD(D,-2,@EasterDate) set @IndDutchHolliday = 0 -- No day off
				if @LoopDate = DATEADD(D,-2,@EasterDate) set @HolidayName = 'Good Friday'
				if @LoopDate = DATEADD(D,39,@EasterDate) set @IndDutchHolliday = 1
				if @LoopDate = DATEADD(D,39,@EasterDate) set @HolidayName = 'Ascension Day'
				if @LoopDate = DATEADD(D,49,@EasterDate) set @IndDutchHolliday = 1
				if @LoopDate = DATEADD(D,49,@EasterDate) set @HolidayName = '1st day of Pentecost'
				if @LoopDate = DATEADD(D,50,@EasterDate) set @IndDutchHolliday = 1
				if @LoopDate = DATEADD(D,50,@EasterDate) set @HolidayName = '2st day of Pentecost'
				if (@dayNumberMonth = 25 and @MonthNumber = 12) set @IndDutchHolliday = 1
				if (@dayNumberMonth = 25 and @MonthNumber = 12) set @HolidayName = '1ste Christmas Day'
				if (@dayNumberMonth = 26 and @MonthNumber = 12) set @IndDutchHolliday = 1
				if (@dayNumberMonth = 26 and @MonthNumber = 12) set @HolidayName = '1ste Christmas Day'
				if (@dayNumberMonth = 14 and @MonthNumber = 2)  set @IndDutchHolliday = 0 -- No day off
				if (@dayNumberMonth = 14 and @MonthNumber = 2)  set @HolidayName = 'Valentine''s Day'
				if (@dayNumberMonth = 31 and @MonthNumber = 10)  set @IndDutchHolliday = 0 -- No day off
