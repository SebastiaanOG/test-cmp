CREATE OR ALTER PROCEDURE [processed].[sp_load_dyn_tenderteamrole]
...
BEGIN
    ...
    DECLARE
        ...
        @table NVARCHAR(60) = 'dyn_tenderteamrole',
    ...
    BEGIN TRY
        ...
        IF OBJECT_ID('tempdb..#temp_dyn_tenderteamrole') IS NOT NULL
            BEGIN
                DROP TABLE #temp_dyn_tenderteamrole
            END

        CREATE TABLE #temp_dyn_tenderteamrole
        (
            [ak_tenderteamrole] NVARCHAR(36),
            [name] NVARCHAR(100),
        ...
        INSERT INTO #temp_dyn_tenderteamrole
        SELECT
        ...
        FROM [staged].[dyn_EntityTenderTeamRole]
        ...
        UPDATE [processed].[dyn_tenderteamrole]
        SET
            [dwh_valid_to] = DATEADD(DAY, -1, @process_run_date),
            [dwh_process_run_id] = @process_run_id,
            [dwh_active] = 0
        FROM #temp_dyn_tenderteamrole AS [T]
        LEFT JOIN [processed].[dyn_tenderteamrole] AS [P] ON [T].[ak_tenderteamrole] = [P].[ak_tenderteamrole]
        WHERE
        ...
        SET
            ...
            [dwh_process_run_id] = @process_run_id,
            [dwh_active] = 0
        FROM [processed].[dyn_tenderteamrole] AS [P]
        LEFT JOIN #temp_dyn_tenderteamrole AS [T] ON [T].[ak_tenderteamrole] = [P].[ak_tenderteamrole]
        WHERE
            [T].[ak_tenderteamrole] IS NULL
            AND [P].[dwh_active] = 1
        SELECT @deleted = @@ROWCOUNT

        --  Insert new records + insert records that were updated in the source
        INSERT INTO [processed].[dyn_tenderteamrole]
        (
            [dwh_valid_from],
            [dwh_valid_to],
            [dwh_active],
