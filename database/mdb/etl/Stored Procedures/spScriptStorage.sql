CREATE PROCEDURE [etl].[spScriptStorage]
...
begin
        ...
        LEFT    JOIN    etl.MetadataStructure C ON S.TABLE_NAME = C.EntityName
...
,       '
			...
			WHERE 1=1
			AND [etl].[fnCreateTableName](t.SystemName, t.EntityName) = [etl].[fnCreateTableName](@lSourceSchema, @lSourceTable))
------------------------------------------------------------------------------------------------------------------------------------------
/*
...
------------------------------------------------------------------------------------------------------------------------------------------
		...
		LEFT JOIN etl.MetadataStructure  M ON M.EntityName = T.EntityName
...
SELECT @INSQL = '' 
        ...
        ,       @INSQL = @INSQL + @lCR + 'From ' + [etl].[fnCreateTableName](@lSourceSchema, @lSourceTable)
        ,       @INSQL = @INSQL + @lCR + ''
